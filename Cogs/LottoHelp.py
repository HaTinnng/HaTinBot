...
class LottoHelp(commands.Cog):
    ...
    async def lotto_help(self, ctx):
        ...
        embed.add_field(name="🛒 복권 구매", value="`#복권구매 n` - 복권을 n장 구매 (1장 = 20,000원, 최대 100장)\n `#복권구매 다/전부/올인` - 구매가능한 만큼 최대한 복권을 구매", inline=False)
        embed.add_field(name="🎯 당첨 결과 확인", value="`#복권결과` - 지난 주, 이번 주 당첨 번호 확인", inline=False)
        embed.add_field(name="🎟 내 복권 조회", value="`#복권확인` - 내가 구매한 복권 확인", inline=False)
        embed.add_field(name="⏳ 복권 추첨", value="매주 **일요일 21:00 (KST)** 자동 추첨", inline=False)

        embed.add_field(
            name="💰 당첨금 안내",
            value=(
                "**1등:** 1,000,000,000원 (6개 일치)\n"
                "**2등:** 50,000,000원 (5개 일치)\n"
                "**3등:** 5,000,000원 (4개 일치)\n"
                "**4등:** 50,000원 (3개 일치)\n"
                "**꽝:** 0원"
            ),
