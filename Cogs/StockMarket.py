...
class StockMarket(commands.Cog):
    ...
    def update_stocks(self):
        """
        모든 주식의 가격을 19.58% ~ -17.12% 변동폭 내에서 변동합니다.
        ...
        for stock in cursor:
            ...
            percent_change = random.uniform(-17.12, 18.58)  # 모든 주식 동일 변동폭 적용
    ...
    async def ranking_ansi(self, ctx):
        ...
        1등은 주황색 배경, 2~3등은 노란색 배경, 4~5등은 초록색 배경으로 표시하며,
        나머지는 색상 없이 출력합니다. 금액은 소수점 없이(.0f) 표시됩니다.
        
        (PC용 Discord 클라이언트에서 ANSI 색상 코드가 적용됩니다.)
        """
        # 1. 유저 자산 계산
        ranking_list = []
        for user in self.db.users.find({}):
        ...
        # 3. ANSI 이스케이프 시퀀스로 배경색만 적용하여 문자열 생성
        lines = []
        lines.append("---- 랭킹 TOP 10 ----\n")
        for idx, (username, total) in enumerate(top_10, start=1):
            if idx == 1:
                # 1등: 주황색 배경 (ANSI 256 컬러 코드 202)
                line = f"\u001b[48;5;202m{idx}. {username} : {total:,.0f}원\u001b[0m"
            elif idx in [2, 3]:
                # 2~3등: 노란색 배경 (ANSI 256 컬러 코드 226)
                line = f"\u001b[48;5;226m{idx}. {username} : {total:,.0f}원\u001b[0m"
            elif idx in [4, 5]:
                # 4~5등: 초록색 배경 (ANSI 256 컬러 코드 46)
                line = f"\u001b[48;5;46m{idx}. {username} : {total:,.0f}원\u001b[0m"
            else:
                # 그 외: 색상 없이 출력
                line = f"{idx}. {username} : {total:,.0f}원"
            lines.append(line)

        # 4. ANSI 코드 블록으로 감싸서 전송 (```ansi)
        ansi_content = "```ansi\n" + "\n".join(lines) + "\n```"
