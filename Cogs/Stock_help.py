...
class StockHelp(commands.Cog):
    ...
    async def Stock_help(self, ctx):
        ...
        embed.add_field(name="❓ **주식은**", value=(
            ...
            "예금을 하면 매일 1번씩 예금금의 0.5%만큼 이자를 지급합니다.(소수점이하 버림처리)\n"
            "대출 시 최대 5,000,000원까지 빌릴 수 있으며 빌린시간부터 24시간뒤에 복리로 3%이자가 계산됩니다.\n"
            "시즌은 이번달 28일 0시 10일에 시즌 종료가 되고 TOP3에게는 칭호가 주어집니다! 칭호는 영원히 유지가 됩니다.\n"
            "주식이 종료되고 휴식 후 다음달 1월 0시 10분에 다시 주식이 열리며 새로운 시즌이 시작됩니다.\n"
            "과연 누가 1등을 차지하게 될까요???. \n"
            "**구매한 주식에 대한 책임은 본인에게 있습니다!!!**\n"
        ), inline=False)

        embed.add_field(name="💰 **기본 명령어**", value=(
            "`#주식참가, #주식참여, #주식시작) [닉네임]` - 주식 게임에 참가합니다. 프로필과 랭킹에 들어갈 닉네임을 꼭 적어주셔야 합니다!\n"
            ...
            "`#예금 all/전부/올인/다/풀예금` - 자신이 가진 모든 현금을 은행에 저장합니다.\n"
            "`#출금 n` - n만큼 은행에서 돈을 출금합니다.\n"
            "`#출금 all/전부/올인/다/풀출금` - 예금한 모든 돈을 출금합니다.\n"
            "`#대출 n` - n만큼 은행에서 돈을 대출합니다.\n"
            "`#대출 all/전부/올인/다/풀대출` - 대출할 수 있는 만큼 돈을 빌립니다.\n"
            "`#대출상환, #상환 n` - 빌린 대출금을 n만큼 상환합니다.\n"
            "`#대출상환, #상환 all/전부/올인/다/풀상환` - 빌린 대출금을 전부 상환합니다.\n"
            "`#뉴스, #news` - 3~5 종목정도 다음변동에 전혀 관련없는 뉴스가 출력됩니다.\n"
        ), inline=False)

        embed.add_field(name="📈 **주식 거래 명령어**", value=(
            "`#주식구매, #매수 [종목명] [수량]` - 해당 주식을 지정 수량만큼 구매합니다.\n"
            "`#주식구매, #매수 [종목명] all/전부/올인/다/풀매수` - 전 재산으로 최대 수량을 구매합니다.\n"
            "`#주식구매, #매수 all/전부/올인/다/풀매수` - 전 재산으로 랜덤한 수량으로 랜덤한 주식을 구매합니다.\n"
            "`#주식판매, #매도 [종목명] [수량]` - 해당 주식을 지정 수량만큼 판매합니다.\n"
            "`#주식판매, #매도 [종목명] all/전부/올인/다/풀매도` - 입력한 종목의 보유 주식을 모두 판매합니다.\n"
            "`#주식소각, #소각 [종목명] [수량]` - 해당 주식을 지정 수량만큼 소각합니다. 돈을 받지 않고 주식을 소각합니다\n"
            "`#주식소각, #소각 all/전부/올인/다/풀소각` - 보유한 모든 주식을 모두 소각합니다.\n"
        ...
        embed.add_field(name="📊 **기타 정보 명령어**", value=(
            ...
            "`#다음시즌` - 다음 시즌의 시작시간 및 종료시간과 남은 시간을 확인합니다.\n"
            "`#시즌결과` - 기록된 시즌명과 시즌기간을 보여줍니다.\n"
            "`#시즌결과 [시즌명]` - 해당 시즌의 TOP3의 이름과 총자산을 보여줍니다.\n"
        ), inline=False)
